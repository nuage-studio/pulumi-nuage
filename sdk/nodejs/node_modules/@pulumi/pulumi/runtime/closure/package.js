...
function getPackageDefinition(path) {
    try {
        const directories = path.split(upath.sep);
        let last = undefined;
        let lastFullPath = undefined;
...
function getAllLeafStrings(objectOrPath, opts) {
...
function makeRule(srcPattern, modPattern) {
...
function isConditionalSugar(exports, name) {
    ...
    for (const key of Object.keys(exports)) {
        if (isSugar && key.startsWith(".")) {
            throw new Error(`${name}:package.json "exports" cannot contain some keys starting with "." and some not.` +
...
class ModuleMap {
    constructor(name, exports, opts) {
        ...
        for (const [modPath, objectOrPath] of Object.entries(exports)) {
            const modName = name + modPath.slice(1);
...
*/
function getModuleFromPath(path, packageDefinition, opts = { isRequire: true }) {
